    ...
    <div className="min-h-screen bg-[#050505] flex selection:bg-white selection:text-black">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 bg-black z-40 pt-24 px-6 overflow-y-auto">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 mb-6">{lang === 'EN' ? 'Projects' : 'Projekty'}</p>
          <nav className="space-y-1">
            {projects.map((project, index) => (
              <button
                key={index}
                onClick={() => handleProjectSelect(index)}
                className={`w-full text-left p-6 transition-all border border-zinc-900 ${selectedProject === index
                  ? 'bg-white text-black'
        ...
        <div className={`max-w-6xl mx-auto p-6 md:p-16 lg:p-20 transition-all duration-500 ${isAnimate ? 'opacity-100' : 'opacity-0'}`}>
          {activeView === 'about' ? (
            <div className="pt-20">
              ...
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-white mb-16 tracking-tighter leading-none uppercase whitespace-nowrap">
                {lang === 'EN' ? 'Sebastian Zdziebko' : 'Sebastian Zdziebko'}
              </h1>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-20">
                <div className="lg:col-span-8">
                  <p className="text-lg md:text-xl text-zinc-400 leading-relaxed font-medium mb-12 tracking-wide">
                    {lang === 'EN'
                      ? 'I am an aspiring Gameplay Developer focused on creating gameplay mechanics in an optimal and accessible way. My primary tool is Unreal Engine, but I also possess a programming background gained through projects in C++, Java, and Python.'
                      : 'Jestem początkującym Gameplay Developerem, skupionym na tworzeniu mechanik rozgrywki w optymalny i przystępny sposób. Moim głównym narzędziem jest Unreal Engine, ale posiadam również zaplecze programistyczne zdobyte podczas pracy nad projektami w C++, Javie oraz Pythonie.'}
                  </p>
                  <p className="text-lg md:text-xl text-zinc-400 leading-relaxed font-medium mb-12 tracking-wide">
                    {lang === 'EN'
                      ? 'In my games, I prioritize universal and modular solutions that allow for easy future expansion of mechanics. I am currently intensively expanding my knowledge of optimization, code readability, and industry standards. I am seeking my first commercial experience and the opportunity to contribute to an engaging project.'
                      : 'W moich grach stawiam na uniwersalne i modularne rozwiązania, które pozwalają na łatwą rozbudowę mechanik w przyszłości. Obecnie intensywnie poszerzam swoją wiedzę z zakresu optymalizacji, czytelności kodu oraz branżowych standardów. Poszukuję pierwszej szansy na komercyjne doświadczenie i dołożenie swojej cegiełki do angażującego projektu.'}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="lg:col-span-8">
                  {activeView === 'project' ? (
                    <>
                      ...
                      <div className="text-lg md:text-xl text-zinc-400 leading-relaxed font-medium mb-20 tracking-wide space-y-6">
                        {currentProject.description[lang].split('\n\n').map((paragraph, index) => {
                          const [title, content] = paragraph.split(':');
                          if (content) {
                            return (
                              <p key={index}>
                                <span className="text-zinc-200 font-bold">{title}:</span>
                                <span className="font-light text-zinc-400">{content}</span>
                              </p>
                            );
                          }
                    ...
                    <div className="space-y-12">
                      <div className="flex items-center gap-4">
                        <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-white whitespace-nowrap">
                          {currentProject.blueprintLabel[lang]}
                        </h2>
                        <div className="h-px w-full bg-zinc-900"></div>
                      </div>
          ...
          <footer className="pt-20 border-t border-zinc-900 flex flex-col md:flex-row justify-between items-center gap-10">
            <div className="flex flex-col md:flex-row gap-8 md:gap-16">
              <div>
                ...
                <a href="mailto:zdziebko.sebastian7@gmail.com" className="text-white font-bold text-sm hover:text-zinc-300 transition-colors tracking-widest">zdziebko.sebastian7@gmail.com</a>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-600 mb-2">
                  {lang === 'EN' ? 'Tel' : 'Tel'}
                </p>
                <p className="text-white font-bold text-sm tracking-widest">+48 798 008 753</p>
              </div>
            </div>
          </footer>
        </div>
